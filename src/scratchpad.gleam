...
pub fn render(node: Node(MarkdownNode)) -> String {
  let children =
    node.children
    |> list.map(render)
    |> string.concat()
  ...
  case node.value {
    Document -> children
    Quote -> "<blockquote>" <> children <> "</blockquote>"
...
pub fn render_nakai(node: Node(MarkdownNode)) -> html.Node(a) {
